...
def eval_coder(specs, *, coder_fn, grade):
    """Coder alone implements each task; tests decide pass/fail."""
    rows = []
    for spec in specs:
        out = coder_fn(spec)
        diff = out.get("diff", "")
...
def eval_reviewer(cases, *, review_fn):
    """Reviewer alone judges (task, diff) cases; truth is each case's test-based label."""
    rows = []
    for case in cases:
        out = review_fn(case)
        rows.append({
...
def eval_loop(specs, *, loop_fn, grade, judge=None):
    """The full coder↔reviewer inner loop. Returns (rows, traces).
...
def eval_workflow(specs, *, workflow_fn, grade):
    """The full workflow turn. Tests decide pass/fail."""
    rows = []
    for spec in specs:
        out = workflow_fn(spec)
        diff = out.get("diff", "")
        g = grade(spec["task_id"], diff)
        rows.append({
            "task_id": spec["task_id"],
