"""agents_adapter.py — drive the REAL workflow agents for eval/trainer.
...
from langbridge_code.eval import bench
from langbridge_code.training.optimizer_trace import trace_to_loop_rounds_from_path
...
def _run_layer(worktree, layer, task, context="", model=None, timeout=EVAL_LAYER_TIMEOUT_SECONDS):
    ...
    proc = subprocess.run(
        [sys.executable, "-m", "langbridge_code.eval._run_layer"],
