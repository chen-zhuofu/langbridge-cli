...
A loop trace is the record of one inner coder↔reviewer loop:
  ...
  {
    "task": str, "worker": "coder",
    "rounds": [
       {"round": int, "diff": str, "worker_report": str,
        "reviewer_report": str, "approved": bool, "verdict": "pass"|"needs_work"|"fail",
        "comments": str, "pushed_back": bool, "reviewer_used_tools": bool}
    ],
...
mapped onto coder and reviewer:
...
def _requested_changes(r):
    """True if this round's review asked for more work (did not pass)."""
    return not r.get("approved")


def responsiveness(trace):
    """Mechanical: after each reviewer change-request, did the diff change next round?"""
...
def alignment(trace, judge=None):
    """LLM-judged: did each next diff move toward what the reviewer asked for?"""
    rounds = trace.get("rounds", [])
    if judge is None:
...
def calibration(trace):
    """Was the reviewer's final verdict calibrated against the correctness oracle?"""
...
def _round_is_silent(r):
    """Reviewer asked for changes but gave no substantive comment."""
    return _requested_changes(r) and not (r.get("comments") or "").strip()


def _round_is_rubber_stamp(r):
    """Reviewer approved without running any tool to check."""
    used_tools = r.get("reviewer_used_tools", r.get("l3_used_tools"))
    return r.get("approved") and used_tools is False
...
def batch_patterns(traces, judge=None, min_tasks=2, low_score=0.5):
    ...
    flag(silent, "reviewer_silence", "Reviewer asked for changes without saying what to fix.")
    flag(rubber, "reviewer_rubber_stamp", "Reviewer approved without running any check.")
    flag(reward_hack, "reward_hack", "Reviewer passed code the hidden tests reject.")
    flag(false_block, "false_block", "Reviewer blocked code the hidden tests accept.")
    flag(low_resp, "coder_unresponsive", "Coder ignored reviewer change-requests.")
    flag(low_algn, "coder_misaligned", "Coder changed code but missed the ask.")
    for comment, k in comment_freq.most_common(5):
        if k >= min_tasks:
            flagged.append({"pattern": "repeated_comment", "tasks": k,
                            "note": f"Reviewer keeps repeating: {comment!r}"})
    return flagged
