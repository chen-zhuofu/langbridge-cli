...
- Continuation requests ("继续", continue) — resume the existing plan; do not
  re-plan unless the user wants a new project.
...
- The planner writes the initial plan (update_plan) and refines failed steps; you mark
  progress with check_subtask after each successful agent_worker — do not ask the planner
  to update checkboxes.
- Pass one unchecked subtask per agent_worker prompt (from read_plan). Do not merge the
  whole plan into one worker call.
...
5. When agent_worker review passes, call check_subtask for that item. If all_complete=false,
   read_plan and dispatch the next subtask — do not tell the user the project is fully done.
...
7. When check_subtask returns all_complete=true, summarize full results for the user."""
