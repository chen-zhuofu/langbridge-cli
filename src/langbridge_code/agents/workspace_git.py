"""Git checkpoints for coder tasks.

Each task attempt snapshots HEAD first. A passing reviewer review is committed; a
failed attempt resets the workspace to the snapshot so half-finished code does not
...
def commit_task(label, task, cwd=None):
    """Stage and commit workspace changes for a finished task."""
    if not _is_repo(cwd):
        return None
